...
class ApiService extends BaseApiService {
  ...
  getMessageStatistics(request) {
    return this.get([rootPath.message, subPath.statistics]).then((res) => this.showMessage(res));
  }

  changeToRead(request) {
    return this.put([rootPath.message], { ids: request, status: 1 }).then((res) =>
      this.showMessage(res)
    );
  }

  // class schedule
  getClassSchedule(request) {
    return this.get([rootPath.class, subPath.schedule], request).then((res) =>
      this.showMessage(res)
    );
  }
  ...
  // get profile
  getProfileByUserId(request) {
    return this.get(rootPath.profile, request).then((res) => this.showMessage(res));
  }

  updateProfile(request) {
    console.log('%c [ request ]', 'font-size:13px; background:pink; color:#bf2c9f;', request);

    return this.put(
      [rootPath.profile, storage.getUserInfo()['role'], storage.getUserInfo()['userId']],
      { ...request }
    ).then((res) => this.showMessage(res));
  }
